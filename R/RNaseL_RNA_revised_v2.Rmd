...
```{r}
# Adjust path to files accordingly
RAW_READ_COUNTS_FILE = "../data/cdsrna_round.csv"
METADATA_FILE = "../data/Metadata.txt"

all <- read.delim2(RAW_READ_COUNTS_FILE , sep = ",", header = TRUE, row.names = 1, comment.char = c("#"))
all$AK098M<-as.double(all$AK098M)
all$AK102M<-as.double(all$AK102M)
all$AK117M<-as.double(all$AK117M)
all$AK123M<-as.double(all$AK123M)
all$AK133M<-as.double(all$AK133M)
all$AK134M<-as.double(all$AK134M)
all$AK135M<-as.double(all$AK135M)
all$AK136M<-as.double(all$AK136M)
all$AK137M<-as.double(all$AK137M)
all$AK138M<-as.double(all$AK138M)
all<-round(all)
...
# Load metadata
metadata <- read.delim2(METADATA_FILE , sep = "\t", row.names = 1, header = T)
